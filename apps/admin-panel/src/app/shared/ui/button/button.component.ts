...
export class ButtonComponent {
  ...
  get classes(): string {
    const base =
      'font-sans font-medium rounded-lg text-sm px-5 py-2.5 text-center focus:ring-4 focus:outline-none transition-all duration-300 uppercase tracking-wider flex items-center justify-center gap-2';
    const width = this.fullWidth() ? 'w-full' : 'inline-flex';
    const disabled =
