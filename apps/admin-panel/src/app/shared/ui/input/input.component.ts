...
export class InputComponent implements ControlValueAccessor {
  ...
  inputClasses = computed(() => {
    const base =
      'block py-2.5 px-0 w-full text-sm bg-transparent border-0 border-b-2 appearance-none focus:outline-none focus:ring-0 peer transition-colors duration-300';
    const normal =
      'text-surface-900 border-surface-300 focus:border-primary-600';
    const errorState =
      'text-red-900 border-red-600 focus:border-red-600 dark:text-red-400 dark:border-red-500';
